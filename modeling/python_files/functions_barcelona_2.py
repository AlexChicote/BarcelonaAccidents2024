...
metrics={'recall':recall_score,'precision':precision_score,'accuracy': accuracy_score,'roc_auc':roc_auc_score}
models_dict = {'dummy': DummyClassifier(strategy='stratified'),
    'logreg': LogisticRegression(C= 1e9, random_state=random_state,class_weight='balanced',max_iter=500,solver='newton-cholesky'),
'random_forest': RandomForestClassifier(class_weight='balanced', max_depth=5, n_estimators=10, max_features=3, random_state=random_state), 
'gbc': GradientBoostingClassifier(random_state=random_state, learning_rate=0.01,min_samples_split = 500,\
...
oversamplings={'ADASYN': over.ADASYN(random_state=random_state),
              ...
              #'SMOTEN': over.SMOTEN(random_state=random_state), NO need when using ColumnTransformer
            #'SMOTENC':over.SMOTENC(categorical_features=categorical_features,random_state=random_state),NO need when using ColumnTransformer
              #'SVMSMOTE': over.SVMSMOTE(random_state=random_state), Too slow
               #'KMeansSMOTE': over.KMeansSMOTE(random_state=random_state,kmeans_estimator=3),
